...
from precompute.precompute_utils import plot_torques
...
contact_phases = [contact_phase_quadru] * int(T_ds)
...
for t in range(n_steps):
    ...
    # comparison
    if t in comp_times:
        res_torques = np.array(mpc.us)
        plot_torques(res_torques)
        plot_torques(uref[t:t+T])
        time = np.arange(np.shape(force_FL)[0])
        plt.plot(time, force_FL)
        plt.grid(True)
        plt.show()
